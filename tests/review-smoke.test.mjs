...
// features=[admin-ui, webhook-handlers, background-jobs, email-sender, customer-facing-ui, third-party-integrations], sync=[products, orders, customers]).
...
test("sync layer wires per-resource webhooks", () => {
  ...
  assert.match(config, /customers\//, "customers webhook topic should be registered");
});
