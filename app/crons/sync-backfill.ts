...
import {
  productsBackfillQuery, ordersBackfillQuery, customersBackfillQuery,
  productNodeToRow, orderNodeToRow, customerNodeToRow,
  ProductNode, OrderNode, CustomerNode,
...
const SYNC_RESOURCES_PICKED = ["products", "orders", "customers"] as const;
...
async function fetchPage(
  ...
  switch (resource) {
    ...
    case "customers": {
      const res = await admin.graphql(customersBackfillQuery, { variables: { first: SYNC_BACKFILL_PAGE_SIZE, after: cursor } });
      const data = (await res.json()) as { data: { customers: { pageInfo: { hasNextPage: boolean; endCursor: string | null }; nodes: CustomerNode[] } } };
      ...
      for (const node of conn.nodes) {
        await upsertResource(env.D1!, {
          ...
          remoteId: node.id,
          remoteUpdatedAt: node.updatedAt,
          row: customerNodeToRow(shop, node, ""),
        });
      }
      return { hasNextPage: conn.pageInfo.hasNextPage, endCursor: conn.pageInfo.endCursor };
    }
