...
import { listCustomers } from "~/lib/sync/api/customers.server";

// Read-only browser for the synced "customers" table. Pagination + search
...
export async function loader({ context, request }: LoaderFunctionArgs) {
  ...
  const result = await listCustomers(env.D1, {
    shop,
    limit: PAGE_SIZE,
...
export default function CustomersDataRoute() {
          ...
          <tbody>
            {data.rows.map((row) => (
              <tr key={row.remoteId} style={{ borderBottom: "1px solid #eee" }}>
                ...
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.firstName)}</td>
                <td className="px-3 py-2 font-mono text-xs">{formatCell(row.lastName)}</td>
