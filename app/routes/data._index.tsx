...
export async function loader({ context, request }: LoaderFunctionArgs) {
  ...
  const counts = {
      ...
      customers: await db.select({ value: count() }).from(schema.customers).where(eq(schema.customers.shop, shop)).then((r) => r[0]?.value ?? 0),
  };
  ...
  return (
    <main style={{ fontFamily: "system-ui", padding: "2rem", maxWidth: 720 }}>
      ...
      <ul style={{ listStyle: "none", padding: 0, lineHeight: 2 }}>
        ...
        <li><Link to="/data/customers">Customers</Link> — {data.counts.customers} rows</li>
      </ul>
