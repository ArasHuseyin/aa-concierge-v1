...
export const customers = sqliteTable("customers", {
  ...
  remoteUpdatedAt: text("remote_updated_at").notNull(),
  syncedAt: integer("synced_at", { mode: "timestamp_ms" }).notNull().$defaultFn(() => new Date()),
  // Tombstoned (not deleted) when customers/redact webhook fires — Shopify
  // requires the data to be erased but we keep the row so future webhooks
  // for the same remote_id can short-circuit instead of re-creating it.
  redactedAt: integer("redacted_at", { mode: "timestamp_ms" }),
}, (table) => ({
  shopRemoteIdx: uniqueIndex("customers_shop_remote_idx").on(table.shop, table.remoteId),
}));
